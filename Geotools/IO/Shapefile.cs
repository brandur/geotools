...
namespace Geotools.IO
{
	...
	public class Shapefile
	{
		...
		public static int ImportShapefile(string filename, string connectionstring, string tableName)
		{
			...
			string sqlSelect = String.Format(System.Globalization.CultureInfo.InvariantCulture, "select * from {0}", tableName);
			...
			int shapefileColumnCount= shpDataTable.Columns.Count;
			int colCount2=  ds.Tables[0].Columns.Count;
			// copy rows from our datatable to the empty table in the DataSet
			int i=0;
			foreach(DataRow row in shpDataTable.Rows)
			{
				DataRow newRow = ds.Tables[0].NewRow();
				object[] values = new object[shapefileColumnCount+1];
				//values[0] =i;
				for (int j=0; j<shapefileColumnCount;j++)
				{
					values[j+1]= row[j];
				}
		...
		public static string CreateDbTable(DataTable table, bool deleteExisting)
		{
		...
		public static string GetDbType(Type type, int length)
		{
			...
			{
				return String.Format(System.Globalization.CultureInfo.InvariantCulture, "nvarchar({0}) ", length);
			}
