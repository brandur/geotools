			...
			foreach ( EdgeEnd e in Edges() ) 
			{
				...
				if ( label.IsArea( geomIndex ) ) 
				{
					...
					if ( rightLoc != Location.Null ) 
					{
						...
						if ( rightLoc != currLoc )
						{
							throw new TopologyException( String.Format(System.Globalization.CultureInfo.InvariantCulture, "Side location conflict " , locStr ));
						}
						if ( leftLoc == Location.Null )
						{
							throw new TopologyException( String.Format(System.Globalization.CultureInfo.InvariantCulture, "Found single null side " , locStr ));
